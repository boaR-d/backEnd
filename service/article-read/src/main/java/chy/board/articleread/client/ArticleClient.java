...
public class ArticleClient {
	...
	@Value("${endpoints.chy-board-article-service.url}")
	...
	public List<ArticleResponse> readAllInfiniteScroll(Long boardId, Long lastArticleId, Long pageSize) {
		try {
			return restClient.get()
				.uri(
					lastArticleId != null ?
						"/v1/article/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s"
							.formatted(boardId, pageSize, lastArticleId) :
						"/v1/article/infinite-scroll?boardId=%s&pageSize=%s"
